...
async function startAudioSeparate(filePath: string, apiKey: string): Promise<string> {
  ...
  formData.append('separate_backing_vocals', 'false');

  const response = await fetch('https://console.oomol.com/api/tasks/tuanzi/audio-separate/start', {
    method: 'POST',
    headers: {
  ...
  if (result.success !== true) {
    throw new Error(`开始音频分离任务失败: ${JSON.stringify(result)}`);
  }
...
async function checkTaskStatus(musicId: string, apiKey: string): Promise<boolean> {
  const response = await fetch(`https://console.oomol.com/api/tasks/tuanzi/audio-separate/status/${musicId}`, {
    headers: {
      'Authorization': `Bearer ${apiKey}`,
    },
...
async function getDownloadUrl(musicId: string, apiKey: string): Promise<string> {
 const response = await fetch(`https://console.oomol.com/api/tasks/tuanzi/audio-separate/download/${musicId}`, {
    headers: {
      'Authorization': `Bearer ${apiKey}`,
    },
    signal: AbortSignal.timeout(10000)
...
export default async function(params: Inputs, context: Context<Inputs, Outputs>): Promise<Outputs> {
  try {
    console.log("starting audio-separate-block");

    // 步骤1: 开始音频分离任务
    const musicId = await startAudioSeparate(params.file, context.OOMOL_LLM_ENV.apiKey);
    ...
    throw new Error(`音频分离任务失败: ${errorMessage}`);
  }
}
